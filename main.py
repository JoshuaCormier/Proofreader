...
class SmokeControlConsoleAuditor:
    def __init__(self, doc_path):
        ...
        # Core verification rules mapping technical anomalies to console feedback directives
        self.static_rules = [
            ...
            (r'Table\s{2,}\d+', "Formatting Error: Excessive whitespace spacing inside Table caption identifier."),
    ...
    def extract_project_profile(self):
        ...
        # Establish building/asset category baseline profile
        ...
        # Establish jurisdiction location baseline profile
    ...
    def print_to_console(self, layer, index, error_type, snippet, message):
        ...
        if len(clean_snippet) > 50:
            clean_snippet = clean_snippet[:47] + "..."
    ...
    def inspect_text_segment(self, text, layer_name, index):
        """Checks text blocks against orthographic patterns, project profiles, and calendar dates."""
        if not text.strip():
            return

        # 1. Evaluate Static Syntax and Broken Reference RegEx Arrays
        ...
        # 2. Evaluate Asset and Boundary Profile Configurations
        ...
        if target_type:
            ...
            for conflict in conflicts:
                if re.search(rf'\b{conflict}\b', text, re.IGNORECASE):
                    # Skip standard occurrences inside table of contents layouts or figure titles
                    if "table of contents" in text.lower() or "figure" in text.lower():
                        continue
        ...
        if target_city and "oklahoma city" in target_city.lower():
            if "mccarran" in text.lower() or "las vegas" in text.lower():
                msg = "Template Boiletplate Contamination: Climate data calculations reference McCarran Airport (Las Vegas) metrics instead of Oklahoma City telemetry."
                self.print_to_console(layer_name, index, "Profile Verification Failure", text, msg)

        # 3. Evaluate Date Parameters against active year criteria
    ...
    def drive_scanning_sequence(self):
        ...
        for idx, paragraph in enumerate(self.doc.paragraphs):
            self.inspect_text_segment(paragraph.text, "Main Body Text Paragraph", idx + 1)
            ...
            for r_idx, row in enumerate(table.rows):
                for c_idx, cell in enumerate(row.cells):
                    for p_idx, paragraph in enumerate(cell.paragraphs):
                        self.inspect_text_segment(paragraph.text,
            ...
            for header in headers:
                if header:
                    for p_idx, paragraph in enumerate(header.paragraphs):
                        self.inspect_text_segment(paragraph.text, f"Section {s_idx + 1} Header Workspace", p_idx + 1)
            ...
            for footer in footers:
                if footer:
                    for p_idx, paragraph in enumerate(footer.paragraphs):
                        self.inspect_text_segment(paragraph.text, f"Section {s_idx + 1} Footer Workspace", p_idx + 1)
        ...
        for b_idx, tx_bx in enumerate(
            ...
            for p_node in tx_bx.findall('{http://schemas.openxmlformats.org/wordprocessingml/2006/main}p'):
                ...
                self.inspect_text_segment(temp_paragraph_wrapper.text, "Cover Page Text Box / Drawing Shape", b_idx + 1)
...
if __name__ == "__main__":
    # Isolate root directory parameters dynamically
